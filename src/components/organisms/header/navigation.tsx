...
import { NAVIGATION } from "@/content/shared";

const renderNavigationItem = ({ variant, label, value, items }: any) => {
  if (variant === "list" && items.length > 0) {
    return (
  ...
  return (
    <div className={cn("col-span-3 hidden items-center justify-center sm:flex", className)}>
      <ReactNavigationMenu.Root className="">
        <ReactNavigationMenu.List className="border border-neutral-700 backdrop-blur-xl bg-black/75 rounded-full center m-0 flex list-none rounded-[6px] bg-white-1234 py-1 px-2 shadow-md">
          {NAVIGATION.map((o) => renderNavigationItem(o))}
