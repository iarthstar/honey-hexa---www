...
const Headline: React.FC<React.HTMLAttributes<null> & { label?: string; title?: string; description?: string }> = ({ className, label = null, title = null, description = null }) => {
  return (
    <PageContainer className={cn("flex flex-col justify-center items-center", className)}>
      {label && <Text as="span" size="xs" className="flex flex-row items-center gap-2 text-white border border-blue-900 bg-blue-900 rounded-full px-3 py-1 uppercase">{label}</Text>}
      ...
      {description && <Text size="base" className="text-neutral-100 text-center mt-2 max-w-2xl drop-shadow-md">{description}&nbsp;</Text>}
    </PageContainer>
  )
}
